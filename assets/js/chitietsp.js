...
function phanTich_URL_chiTietSanPham() {
    ...
    document.title = nameProduct + ' - Smart Store';
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        case 'tragop':
            ...
            return `Khách hàng có thể mua trả góp sản phẩm với ` + span + `với thời hạn vĩnh viễn kể từ khi mua hàng.`;
        ...
        case 'moiramat':
            return `Khách hàng sẽ được thử máy miễn phí tại cửa hàng. Có thể đổi trả lỗi trong vòng 1 tỉ ngày.`;

        case 'giareonline':
            var del = stringToNum(sp.price) - stringToNum(sp.promo.value);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Sản phẩm sẽ được giảm ` + span + `₫ khi thanh toán bằng thẻ ngân hàng.`;

        default:
            var span = `<span style="font-weight: bold">Iphone 14 Pro Max</span>`;
            return `Cơ hội trúng ` + span + ` khi mua hàng.`;
    }
}
...
function suggestion(){
    ...
    .map(sanPham => {
        ...
        // Tính điểm cho sản phẩm được gợi ý, điểm cao hơn sẽ được gợi ý
        let diem = 0;
